...
def test_policy_base_class():
    ...
    with pytest.raises(NotImplementedError) as excinfo:
        policy.get_probs_from_observations_and_actions(observations,actions,behavior_action_probs) 
...
def test_Softmax():
    ...
    probs = sm.get_probs_from_observations_and_actions(observations, actions, behavior_action_probs)
...
def test_Fourier():
    ...
    num_features = basis.calculate_num_features(order=2, max_coupled_vars=2, num_obs_dims=3)
